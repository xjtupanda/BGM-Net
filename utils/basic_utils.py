...
class BigFile:
    '''
        Maintain two 'dict':
            list: self.names: [clip_name] . e.x. castle_s01e01_seg02_clip_00_0
            dict: self.name2idx: {clip_name: id}  id in [0, total_images-1]
    '''
    ...
    def read(self, requested, isname=True):
        ...
        index_name_array.sort(key=lambda v: v[0])   # list of tuple (idx, clip_name)
        ...
        for next in sorted_index[1:]:
            ...
            # print next, move
            fr.seek(move, 1)    # second arg. 0 by default: from file start, 1: from current pointer, 2: from EOF
            res.fromfile(fr, self.ndims)
            previous = next

        fr.close()
        # [clip_name], [feature, each of ndims]
