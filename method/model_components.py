...
class clip_nce(nn.Module):
    ...
    def forward(self,labels, label_dict, q2ctx_scores=None, contexts=None, queries=None):
        # q2ctx_scores: (#captions=640, N=128)
        # labels: list:(640), sample index in batch [0, 128-1]
        # label_dict: # index_in_batch : [index in label from 0~#captions-1]
        query_bsz = q2ctx_scores.shape[0]   # 640
        vid_bsz = q2ctx_scores.shape[1]     # 128
        diagnoal = torch.arange(query_bsz).to(q2ctx_scores.device)      # (0, 1, 2,..., cap-1)
        t2v_nominator = q2ctx_scores[diagnoal, labels]                  # (640)

        t2v_nominator = torch.logsumexp(t2v_nominator.unsqueeze(1), dim=1)  # doesn't change anything in fact...
        t2v_denominator = torch.logsumexp(q2ctx_scores, dim=1)      # 640
...
class frame_nce(nn.Module):
    ...
    def forward(self, q2ctx_scores=None, contexts=None, queries=None):
        # (#cap=640, 640), None, None
        # first: query embedding. second: corresponding video embedding using key clip attention
        if q2ctx_scores is None:
            ...
            x = q2ctx_scores            # (640, 640)
            device = q2ctx_scores.device
            bsz = q2ctx_scores.shape[0] # (640)

        x = x.view(bsz, bsz, -1)        # (640, 640, 1)
        nominator = x * torch.eye(x.shape[0], dtype=torch.float32, device=device)[:, :, None]   # diagnol of (bsz, bsz, 1) multiple x, 
                                                                                                # extract diagonal. (positive pairs)
        nominator = nominator.sum(dim=1)        # (640, 1)

        nominator = torch.logsumexp(nominator, dim=1)       # (640)

        denominator = torch.cat((x, x.permute(1, 0, 2)), dim=1).view(x.shape[0], -1)    # (640, 1280)
        denominator = torch.logsumexp(denominator, dim=1)                               # (640)
        if self.reduction:
